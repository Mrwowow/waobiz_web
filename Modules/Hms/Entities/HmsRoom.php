...
class HmsRoom extends Model
{
    ...
    {
        // Query to find rooms that are not booked or unavailable for the given type and time period
        $non_booking_rooms = HmsRoom::leftJoin('hms_booking_lines', function ($join) use ($arrival_date_time, $departure_date_time, $t_id) {
            // Join rooms with booking lines to check if they are booked
                ...
                ->where(function ($query) use ($arrival_date_time, $departure_date_time) {
                    $query->whereRaw("transactions.hms_booking_arrival_date_time <= ? AND transactions.hms_booking_departure_date_time >= ?", [$departure_date_time, $arrival_date_time])
                        ...
                        ->where(function ($query) {
                            $query->whereNull('transactions.check_out'); // Room is booked if check_out is NULL
                        });
                })
