<div class="modal-dialog modal-xl" role="document">
    ...
    <div class="modal-footer">
        @if(empty($transaction->check_in))
            <button type="submit" class="tw-dw-btn tw-dw-btn-primary tw-text-white">@lang( 'hms::lang.confirm_check_in' ) </button>
            <button type="button" class="tw-dw-btn tw-dw-btn-neutral tw-text-white" data-dismiss="modal">@lang( 'messages.close' )</button>
        @elseif (!empty($transaction->check_in) && empty($transaction->check_out))
            @if($transaction->payment_status != 'paid')
                <h4>@lang('hms::lang.check_out_payment_help_text')</h4>
            @endif
            <button type="submit" @if($transaction->payment_status != 'paid') disabled @endif class="tw-dw-btn tw-dw-btn-primary tw-text-white">@lang( 'hms::lang.confirm_check_out' ) </button>
            <button type="button" class="tw-dw-btn tw-dw-btn-neutral tw-text-white" data-dismiss="modal">@lang( 'messages.close' )</button>
        @endif
    </div>
    {!! Form::close() !!}
