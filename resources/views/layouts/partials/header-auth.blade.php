    ...
    <div class="row">
        ...
        <div class="tw-absolute tw-top-3 md:tw-top-8 tw-right-4 md:tw-right-10 tw-flex tw-items-center tw-gap-4 md:tw-gap-10"
            ...
            @if (!($request->segment(1) == 'business' && $request->segment(2) == 'register') && $request->segment(1) != 'login')
                <a class="tw-text-black tw-font-medium tw-text-sm md:tw-text-base hover:tw-text-black"
            ...
            <!-- Register  -->
            <div class="tw-border-2 tw-border-black tw-rounded-full tw-h-10 md:tw-h-12 tw-w-24 tw-flex tw-items-center tw-justify-center">
                @if (!($request->segment(1) == 'business' && $request->segment(2) == 'register'))

                    <!-- Register Url -->
                    @if (config('constants.allow_registration'))
                        <a href="{{ route('business.getRegister') }}@if (!empty(request()->lang)) {{ '?lang=' . request()->lang }} @endif"
                            class="tw-text-black tw-font-medium tw-text-sm md:tw-text-base hover:tw-text-black">{{ __('business.register') }}
                        </a>
